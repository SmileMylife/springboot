...
public class QueryMessageController {
    @Autowired
    private IQueryMessageService iQueryMessageService;

    @RequestMapping(value = "/queryMessages", method = RequestMethod.POST)
    @ResponseBody
    public OutputObject queryMessages(@InputObject com.example.springboot.common.bean.InputObject inputObject, OutputObject outputObject) {
        inputObject.getParams().put("dbKey", "proj_busi");
        iQueryMessageService.queryMessage(inputObject, outputObject);
        return outputObject;
    }
    ...
    public void uploadDoc(@RequestParam("file") MultipartFile multipartFile,
        ...
        if (!Arrays.asList(Constants.ALLOW_JSON_FILE.split(",")).contains(fileExt)) {
            throw new TxtException("文件类型只支持.json");
        }

        String jsonStr = new String(bytes);

        inputObject.getParams().put("jsonStr", jsonStr);
        iQueryMessageService.uploadMessage(inputObject, outputObject);
    }
     ...
     * @param outputObject
     */
    @RequestMapping(value = "/downLoadMessage", method = RequestMethod.GET)
...
//        iQueryDocService.downLoadDoc(inputObject, outputObject);
    ...
    @RequestMapping(value = "/lookOnlineForMessage", method = RequestMethod.GET)
